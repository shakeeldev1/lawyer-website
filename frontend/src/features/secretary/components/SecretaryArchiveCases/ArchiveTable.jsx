...
const ArchiveTable = ({ cases, onView, onDelete }) => {
  ...
  const TableRow = ({ c, idx }) => (
    ...
    >
      <td className="p-4 font-semibold text-slate-800 whitespace-nowrap">{c.id}</td>
      ...
      <td className="p-4 font-medium text-slate-800 whitespace-nowrap">{c.caseNumber}</td>
      ...
      <td className="p-4">
        <div className="space-y-1">
          {c.stages.map((s, i) => (
            <div key={i} className="flex items-center gap-2 text-sm text-slate-700 whitespace-nowrap">
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className={`overflow-x-auto w-[330px]  text-left border-collapse ${sidebarOpen ? "md:w-[489px] lg:w-[1050px] ":"md:w-[680px] lg:w-full" }`}>
...
<tbody className="divide-y divide-slate-200">
  {cases.map((c, idx) => (
    <tr key={c.id} className={`transition-all duration-200 hover:bg-slate-50 ${idx % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
      ...
      <td className="p-4 text-slate-800 whitespace-nowrap">{c.additionalInfo}</td>
      <td className="p-4 whitespace-nowrap">
        <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs font-semibold whitespace-nowrap">
          {c.caseType}
        </span>
