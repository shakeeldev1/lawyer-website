...
const UserDeleteModal = ({ show, onClose, onDelete }) => {
  if (!show) return null;
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur flex items-center justify-center z-70">
      <div className="bg-white rounded-xl shadow-lg p-6 w-[90%] max-w-sm border border-gray-200">
        ...
        <div className="flex justify-end gap-3">
          ...
          <button onClick={onDelete} className="px-4 py-2 text-sm bg-red-600 text-white rounded hover:bg-red-700">Delete</button>
        </div>
      </div>
    </div>
  );
