...
const CasesTable = ({ cases, onView, onDelete }) => {
  return (
    <div className="bg-white md:bg-white text-[#24344f] shadow-2xl rounded-2xl border border-[#fe9a00]/20 overflow-hidden">
      {/* ===== Desktop Table View ===== */}
      ...
      <div className="block lg:hidden p-4 space-y-4">
        {cases.map((c) => (
          <div
            key={c.id}
            className="border border-[#fe9a00]/30 rounded-xl p-4 shadow-md bg-white hover:shadow-lg transition-all duration-200"
          >
            <div className="flex justify-between items-center mb-2">
              <h3 className="font-semibold text-[#24344f]">
                Case #{c.caseNumber}
              </h3>
              <span
                className={`px-3 py-1 rounded-full text-xs font-medium ${
            ...
            <div className="space-y-1 text-sm text-[#24344f]">
              ...
              <p>
                <span className="font-medium">Lawyer:</span> {c.lawyer}
              </p>
              <p>
            ...
            <div className="mt-3 flex justify-end gap-3">
              <button
                onClick={() => onView(c)}
                className="flex items-center justify-center gap-2 text-[#fe9a00] border border-[#fe9a00] hover:bg-[#fe9a00] hover:text-white px-3 py-1.5 rounded-full font-medium transition-all duration-200"
              >
                <Eye className="w-4 h-4" /> View
              </button>
              ...
              <button
                onClick={() => onDelete(c)}
                className="flex items-center justify-center gap-2 text-red-600 border border-red-600 hover:bg-red-600 hover:text-white px-3 py-1.5 rounded-full font-medium transition-all duration-200"
              >
                <Trash2 className="w-4 h-4" /> Delete
              </button>
            </div>
