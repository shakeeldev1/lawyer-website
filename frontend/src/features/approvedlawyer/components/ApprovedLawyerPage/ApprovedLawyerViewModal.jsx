    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-6">
      ...
      >
        ...
        <div className="sticky top-0 bg-gradient-to-r from-gray-900 to-gray-800 text-white rounded-t-2xl px-5 py-5 flex justify-between items-center">
          <div>
            <h2 className="text-lg sm:text-xl font-bold">{selectedCase.caseName}</h2>
        ...
        <div className="bg-gray-50 border-b border-gray-200 px-5 py-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-700">
          <div className="flex gap-2"><span className="font-semibold">Client:</span> {selectedCase.client.name}</div>
          <div className="flex gap-2"><span className="font-semibold">Email:</span> {selectedCase.client.email}</div>
          <div className="flex gap-2"><span className="font-semibold">Phone:</span> {selectedCase.client.phone}</div>
          <div className="flex gap-2"><span className="font-semibold">Address:</span> {selectedCase.client.address}</div>
          <div className="flex gap-2"><span className="font-semibold">Case Type:</span> {selectedCase.caseType}</div>
          <div className="flex gap-2"><span className="font-semibold">Additional Info:</span> {selectedCase.client.additionalInfo || "N/A"}</div>
        </div>

          {/* SHOW MODIFICATION MESSAGE */}
        {selectedCase.modificationMessage && (
          <div className="mt-4 bg-yellow-50 border-l-4 border-yellow-400 p-3 rounded">
            <h4 className="font-semibold mb-1">Modification Message:</h4>
            <p>{selectedCase.modificationMessage}</p>
          </div>
        )}

        {/* Stage Timeline — FIXED LOCATION */}
        <div className="p-5 space-y-5">
          ...
          <div className="relative flex items-center justify-between mt-4">
            ...
            {STAGES.map((stage, idx) => {
              const currentIdx = STAGES.indexOf(selectedCase.currentStage);
                ...
                <div key={idx} className="relative flex flex-col items-center w-full">
                  <div
                    className={`w-9 h-9 flex items-center justify-center rounded-full border-2 transition-all duration-300
                      ${
                        isCurrent
                          ? "bg-amber-500 border-amber-600 text-white shadow-lg scale-110"
                          : isCompleted
        ...
        {selectedCase.modificationMessage && (
          <div className="mt-4 bg-yellow-50 border-l-4 border-amber-400 p-3 text-gray-800 rounded">
            <h4 className="font-semibold mb-1">Modification Message:</h4>
            <p>{selectedCase.modificationMessage}</p>
          </div>
        )}
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-200 rounded-b-2xl px-5 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button
              className="flex items-center gap-2 px-5 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 transition"
            ...
            <button
              className="flex items-center gap-2 px-5 py-2 bg-amber-500 text-white rounded-lg font-medium hover:bg-amber-600 transition"
